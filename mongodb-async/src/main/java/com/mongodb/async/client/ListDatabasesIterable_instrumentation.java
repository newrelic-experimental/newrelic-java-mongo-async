...
abstract class ListDatabasesIterable_instrumentation<TResult> {
    ...
    public void first(SingleResultCallback<TResult> callback) {
    	...
    	if(!(callback instanceof NRCallbackWrapper)) {
    		...
    		wrapper.params = DatastoreParameters.product(DatastoreVendor.MongoDB.name())
    				.collection("")
    				.operation(MongoUtil.OP_LIST_DATABASES)
    				.build();
    		wrapper.segment = NewRelic.getAgent().getTransaction().startSegment(MongoUtil.OP_LIST_DATABASES);
    		callback = wrapper;
    	}
    	Weaver.callOriginal();
    }
