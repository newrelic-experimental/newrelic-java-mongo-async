...
abstract class ListCollectionsIterable_instrumentation<TResult> {
    ...
    public void first(SingleResultCallback<TResult> callback) {
    	...
    	if(!(callback instanceof NRCallbackWrapper)) {
    		...
    		wrapper.params = DatastoreParameters.product(DatastoreVendor.MongoDB.name())
    				.collection("")
    				.operation(MongoUtil.OP_LIST_COLLECTIONS)
    				.noInstance()
    				.databaseName(databaseName)
    				.build();
    		wrapper.segment = NewRelic.getAgent().getTransaction().startSegment(MongoUtil.OP_LIST_COLLECTIONS);
    		callback = wrapper;
    	}
    	Weaver.callOriginal();
    }
